...
// Construire le prompt système
    ...
    // RÈGLES CRITIQUES — priorité absolue sur tout le reste
    systemPrompt += `
## RÈGLES CRITIQUES ABSOLUES

1. FORMAT : Répondre UNIQUEMENT en JSON brut. Commencer par { et finir par }. Aucun texte avant ou après. Aucun markdown. Aucun astérisque.

2. au/vau : Mot précédent finit par E ou I → au. Mot précédent finit par A, O ou U → vau. Appliquer dans TOUTES les sections de la réponse.

3. ATTRIBUT ADJECTIVAL : "Le/La + nom + est + adjectif" → TOUJOURS E mea + adjectif + te + nom. JAMAIS E + nom + adjectif + teie.

4. POSSESSION OBJET : Objet ordinaire → TOUJOURS ordre A → Nā (jamais Nō). Ex: Nā Hiro te tipi (jamais Nō Hiro te tipi).

5. GENRE : Le reo Tahiti n'a pas de genre grammatical. Ne jamais écrire qu'un terme est "neutre en genre".

6. E + V + nei = FAUTE GRAVE — ne jamais produire.
`;

    // Injecter le glossaire Upstash (priorité absolue)
    if (glossaireUpstash.length > 0) {
      systemPrompt += '\n\n## GLOSSAIRE EXPERT VALIDÉ (priorité maximale — ne jamais contredire)\n';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Renvoyer le JSON parsé directement
    try {
      const result = JSON.parse(text);
      res.status(200).json(result);
    } catch(e) {
      res.status(500).json({ error: 'Réponse IA non parseable: ' + text.slice(0, 200) });
    }
